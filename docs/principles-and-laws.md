...
In core's reference design `PlaybackRuntime` is the application-scoped owner of
the platform player and `PlaybackCoordinator`. **This client uses neither.**
Here `PlaybackProvider` (`src/providers/PlaybackProvider.tsx`), mounted once
